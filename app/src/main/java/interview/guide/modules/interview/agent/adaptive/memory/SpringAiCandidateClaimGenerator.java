...
@Component
public class SpringAiCandidateClaimGenerator implements CandidateClaimGenerator {
  ...
  public CandidateClaimsProposal generate(
    ...
    try {
      ...
      CandidateClaimsProposal proposal = deadlineExecutor.invoke(
          ...
          System.nanoTime() + properties.getClaimDeadline().toNanos(),
          "候选人声明抽取"
      );
      telemetry.modelCallSucceeded("memory_claim_extractor", "CLAIMS", startedNanos);
      return proposal;
      ...
      throw new BusinessException(e.getCode(), "候选人声明抽取失败");
    }
  }

  private String serialize(CandidateClaimExtractionRequest request) {
    try {
      return objectMapper.writeValueAsString(request);
    } catch (JacksonException e) {
      throw new BusinessException(ErrorCode.AI_SERVICE_ERROR, "声明抽取上下文序列化失败", e);
    }
  }
}
