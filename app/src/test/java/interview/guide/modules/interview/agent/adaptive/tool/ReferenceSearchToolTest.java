...
class ReferenceSearchToolTest {
  ...
  @Test
  void rejectsForeignTargetsAndBlankQueries() {
    assertThatThrownBy(() -> tool.read(context(), "foreign", "query")).isInstanceOf(ReadToolValidationException.class);
    assertThatThrownBy(() -> tool.read(context(), "target-0", " ")).isInstanceOf(ReadToolValidationException.class);
  ...
  void filtersByMappedResourcesDeduplicatesAndProducesNoAdoptableSources() {
    ...
    var result = (ReadToolResult.Success) tool.read(context(), "target-0", "query");
  ...
  void distinguishesUnavailableEmptyStaleAndBudgetOmitted() {
    assertThat(tool.read(context(), "target-0", "query")).isInstanceOf(ReadToolResult.Error.class);
    ready();
    when(vectors.similaritySearch(any(SearchRequest.class))).thenReturn(List.of());
    assertThat(tool.read(context(), "target-0", "query")).isInstanceOf(ReadToolResult.Empty.class);
