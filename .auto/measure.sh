...
adaptive_removed_loc=$((baseline_prod_loc - adaptive_prod_loc))
adaptive_prod_files=$(find "$root" -type f -name '*.java' | wc -l | tr -d ' ')
adaptive_spring_components=$(grep -RIlE '^@(Component|Service|Repository|RestController|Controller|Configuration)\b' "$root" --include='*.java' | wc -l | tr -d ' ')
adaptive_test_files=$(find app/src/test/java/interview/guide/modules/interview/agent/adaptive -type f -name '*.java' | wc -l | tr -d ' ')

printf 'METRIC adaptive_removed_loc=%s\n' "$adaptive_removed_loc"
