...
[44-native-tools-agent-runtime-spec.md](./44-native-tools-agent-runtime-spec.md) 记录已确认的原生工具迁移目标（待实施）：保留自有 Runtime 和三个角色，使用 Spring AI 执行原生查询及出题/结束提案工具，删除自定义工具协议。其工具传输与执行目标更新 36、39、42 的对应部分；上述现状不代表迁移已经完成。
...
| [44-native-tools-agent-runtime-spec.md](./44-native-tools-agent-runtime-spec.md) | Spring AI 原生工具与自有 Runtime 改进规格 | 方案已记录，待实施；包含职责、删除范围、迁移与验收要求 |
