...
- **工作轨迹**：当前 ReAct 循环的 thought/action/observation 序列，循环结束即弃，不落库（决策记录除外）。注意它不等于新分类的 Working Memory——后者是从轮次状态提炼的决策性字段（当前事件、未闭合 gap、追问深度），每次决策时从 PG + 本轮裁决结果构造为不可变快照，供出题链路读取，第一期不引入 Redis（34 号 spec §3）；
- **会话记忆**：已完成轮次（问题 + 回答原文 + 工具结果），DB 为事实源。一个已回答的 turn 就是一个事件 Episode（追问会产生新 turn，用 `parentTurnIndex` 关联），是事件记忆的存储单元（34 号 spec §4）；
...
上下文装配器是短期记忆的唯一出口：按角色输出最小上下文（面试官见本维度原文 + 历史维度小结；规划见 JD + 简历 + 画像的"已覆盖主题"）。单次调用输入 token 上限入配置，超限报错不静默截断。

### 5.2 长期记忆（跨会话，M3）

新分类下长期记忆拆为两层：**Episodic Memory**（跨会话事件，每个已回答 turn 一个 Episode，回填规则见 34 号 spec §4.5）与 **Semantic Memory**（能力画像 + 错误模式/回答习惯枚举标签，见 34 号 spec §5）。
...
- 只写结构化字段（枚举、ID 引用、时间戳），LLM 自由文本永不入长期记忆（防跨会话记忆投毒）——**有限破例**：事件的「用户回答摘要」与「结果叙述」是 LLM 自由文本，可入库但只用于前端展示，永不进 prompt、永不进评级链路（34 号 spec §4.3）；
- 记忆写入与面试落库同事务——Episode 例外：异步生成、独立事务、最终一致，不阻断答题链路（34 号 spec §4.4）；
- 公平性防火墙的读侧规则同步生效：长期记忆可影响**选题**，任何评级类字段在 M5 前不存在、M5 后对评估 Agent 硬隔离。
