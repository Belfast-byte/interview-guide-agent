...
| T12 | 已完成 | 删除 `LoadSkillTool`、`RubricLookupTool`、`SandboxSubmitTool`、旧 `AdaptiveAgentTool` 回调协议和 `AgentRoleRegistry/Definition`；旧 ReAct 路径不再装配 Tool，固定 Skill 继续由 `ContextAssembler` 加载，沙箱继续由回答 Application Service 直接提交；删除未启用的题库 MCP 远端优先/fallback。规格中“题库索引无闭环”的前提已过期：本地 `QuestionBankSemanticSearch` 仍被练习推荐真实消费，因此保留为普通业务服务而非 Agent Tool | `AdaptivePackageIsolationTest`、创建、rubric Tool 纵向、旧 ASK 恢复、模型映射、题库普通服务和练习推荐聚焦测试通过；`:app:compileJava` 通过 |
| T13 | 已完成 | 物理删除 WorkState/Patch/Reducer/Policy、ActionIntent 执行与恢复、旧 ReAct gateway/runtime、ToolCall/ToolResultEvent 持久协议及 retry/follow-up API；创建和回答继续由各自短事务纵向服务推进，原大 Application/Persistence 服务收缩为用例委托和事实查询。旧 Tool Result 不再进入 Turn provenance 或 Episode source；沙箱结果只通过 `SandboxExecution` 关联的正式 Evidence 暴露。按票据边界保留历史数据库表列，统一交由 T14 新 migration 删除 | `:app:compileJava`、`:app:compileTestJava` 与全量 `:app:test` 通过（563 tests，49 skipped）；生产源码被删类型零命中 |
| T14 | 已完成 | 新增 V20260927 删除 Intent/WorkState/Patch/ToolCall/ToolResultEvent 表及遗留 FK/列；旧 Tool Result Turn 保留领域内容并迁移为 `AGENT_DECISION`，无法无损映射为候选人能力事实的旧只读 Tool Evidence 显式删除；Plan 及 API 删除 suggested tools/follow-up/tool budget 伪状态，前端删除已下线 follow-up API。同期已暂存的 context 类按真实消费者归位到 action/planning/codeanalysis/memory package，不改变协议 | PostgreSQL 16 + pgvector 空 schema 全迁移、V20260926 升级至 V20260927 及 JPA validate 通过；前端 build 通过；后端单命令触发 60 秒硬超时后按 AppTest/非 Adaptive/Adaptive 核心/Adaptive 记忆持久化四组等价覆盖，全部通过 |
