...
### 审查问题 2、3 修复（2026-09-12）

- 代码 gap 的追问必须沿用来源任务引用；模型输出校验与提交事务均检查，事务以本场、所选维度的正式开放 gap 为准。拒绝提交不会删除已接收答案，也不会留下评估、Episode 或下一轮。
- 历史 TEXT 追问即使漏存任务引用，Episode 仍沿正式评估来源链判断代码反馈权限；评估进行中隐藏理由、gap 和关闭证据，完成后公开，练习正常读取。
- Java 编辑器通过 CodeMirror annotation 区分外部代码同步与实际编辑。初始化不再回写草稿或清除恢复错误；实际编辑继续保存。新增损坏草稿的完整页面回归。
- 本次范围按用户要求仅修问题 2、3；MCP 审查问题未处理。
...
- 候选人在原有 `/workspace` 页面填写 JD、简历并创建面试，首题直接生成 Java 代码改错任务，随后沿用原有会话页面的编辑、提交和审阅链路。
- 候选人创建服务向 Planner 传入代码首题要求，非 CODE_REPAIR 首题明确拒绝；不新增页面、路由或公开请求字段，不另建验收会话。
- 评估与练习模式保留原有反馈公开规则；后续题型继续由 Agent 决策。已有会话不改写，无数据库迁移。
- 真实模型题目质量仍按 CR-08 验收，不因首题顺序调整而视为通过。
- 本轮后端定向测试 25 项、前端代码改错相关测试 32 项全部通过，生产构建通过；覆盖原始 JD/简历传递、两种模式生成代码首题、错误题型拒绝及代码提交。未执行真实模型和浏览器人工验收，构建保留既有 Browserslist/CSS 警告。
- 后续用户验收反馈：创建时报“规划结果包含未知考察重点标识”。已确认由 PlanningTaxonomy 对 Skill 下 focusId 的精确匹配校验触发；尚未取得该次模型输出，具体错误标识未知。创建阶段业务校验失败直接终止，没有反馈模型纠正；新增创建测试模拟了分类校验器，未覆盖真实模型与真实目录组合。此问题仍未修复，作为后续审查项保留。
