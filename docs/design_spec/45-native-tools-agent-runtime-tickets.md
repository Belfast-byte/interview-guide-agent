...
| NATIVE-6 | 独立只读工具限并发 | 共享截止时间、独立失败、调用 ID 配对与原生接线 | DONE |
...
- 用户要求保留共享截止时间、单调用独立失败、原调用 ID 关联；并要求优先使用框架实现。核对项目 Spring AI 2.0.0 实际 API、官方当前 DefaultToolCallingManager 源码及已有社区依赖后，没有可用的框架并发执行器。未升级依赖，也没有恢复旧 Gateway。
- 新增仅由 InterviewAgentLoop 使用的 InterviewToolBatch，隔离批次调度/取消/汇合生命周期，避免把线程管理混入模型决策循环；每个实际调用仍委托原生 manager。默认 `app.interview.adaptive-agent.max-concurrent-read-tools=3`，模型侧允许多工具调用；提案在查询汇合后串行校验、可见证据保持批次前快照。
- 调用运行时异常及本地超时分别返回独立错误 Observation，其余成功响应保留并按原 call ID 和原顺序汇合。共享截止时间耗尽后取消未完任务、关闭作用域阻断迟到结果，Loop 不再调用模型或采用提案；不会把截止失败报告为整轮成功。同批准入/执行顺序不承诺，预算和同参去重由同步作用域保证。
- 新增测试覆盖并发上限、乱序完成/一失败两成功、共享 deadline 与不合作任务迟到隔离、独立本地超时、预算、重复请求、失败兄弟结果回到下一模型后仍能采用成功来源；原 HTTP 合约同步验证 parallel_tool_calls=true，保留真实 SDK 与消息 ID 配对验证。
- 全部 adaptive 回归：385 项，382 通过、3 条件跳过，34s。subagent `review_parallel_tools` 审查无阻塞发现。
- 真实 Provider 回放第一次：独立 RAG 查询通过，完整回放因模型提案被拒后用完 8 步而失败；不记作成功。第二次带临时脱敏诊断回放 2 项全部通过，出现同批 `interview_material_read` 两次调用及 `assessment_read` / `memory_recall` 组合调用，完成顺序与原调用顺序不同，结果均返回并继续产生合法提案；完整回放及无正式提交断言通过。临时结果解析诊断已移除，不让日志解析影响工具成功语义。这些样例证明实际模型/原生链路兼容，不能保证每次模型都给出合法提案，也不作为线上性能提升量化结论。
